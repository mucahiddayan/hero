...
export const addItem = async (listId: string, name: string): Promise<ShoppingItem> => {
    const response = await axios.post(`${API_URL}/lists/${listId}/items`, { name });
    return response.data;
};

export const updateItem = async (itemId: string, bought: boolean): Promise<ShoppingItem> => {
